...
import { accesslint092 } from "./accesslint-092";
...
describe("accesslint-092", () => {
  ...
  it("fails: black text on dark gray background (insufficient contrast)", () => {
    ...
    const violations = accesslint092.run(doc);
  ...
  it("passes: black text on white background", () => {
    ...
    expect(accesslint092.run(doc)).toHaveLength(0);
  });
  ...
  it("fails: ancestor has no-op filter grayscale(0)", () => {
    ...
    expect(accesslint092.run(doc)).toHaveLength(1);
  });
