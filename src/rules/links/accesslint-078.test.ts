...
import { accesslint078 } from "./accesslint-078";


describe("accesslint-078", () => {
  // --- Pass cases ---
  ...
  it("passes: skip link with valid target", () => {
    ...
    expect(accesslint078.run(doc)).toHaveLength(0);
  });
  ...
  it("fails: skip link pointing to nonexistent target", () => {
    ...
    const violations = accesslint078.run(doc);
    expect(violations).toHaveLength(1);
    expect(violations[0].ruleId).toBe("accesslint-078");
