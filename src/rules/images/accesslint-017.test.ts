...
import { accesslint017 } from "./accesslint-017";
...
describe("accesslint-017", () => {
  it("reports object without alternative text", () => {
    ...
    const violations = accesslint017.run(doc);
  ...
  it("passes object with aria-label", () => {
    ...
    expect(accesslint017.run(doc)).toHaveLength(0);
  });
  ...
  it("flags object with only text fallback content (not an accessible name)", () => {
    ...
    expect(accesslint017.run(doc)).toHaveLength(1);
  });
