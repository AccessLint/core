...
import { accesslint089 } from "./accesslint-089";
...
describe("accesslint-089", () => {
  it("reports duplicate IDs referenced by aria-labelledby", () => {
    ...
    expect(accesslint089.run(doc)).toHaveLength(1);
  });
  ...
  it("reports multiple distinct duplicate IDs in one document", () => {
    ...
    expect(accesslint089.run(doc)).toHaveLength(2);
  });
  ...
  it("ignores duplicate IDs not referenced by any accessibility attribute", () => {
    ...
    expect(accesslint089.run(doc)).toHaveLength(0);
  });
