...
import { accesslint034 } from "./accesslint-034";


describe("accesslint-034", () => {
  it("reports empty h1", () => {
    const doc = makeDoc("<html><body><h1></h1></body></html>");
    const violations = accesslint034.run(doc);
  ...
  it("reports empty h2", () => {
    ...
    expect(accesslint034.run(doc)).toHaveLength(1);
  });
  ...
  it("passes heading with text", () => {
    ...
    expect(accesslint034.run(doc)).toHaveLength(0);
  });
  ...
  it("reports multiple empty headings", () => {
    ...
    expect(accesslint034.run(doc)).toHaveLength(3);
  });
