...
import { accesslint023 } from "./accesslint-023";
import { makeDoc } from "../test-helpers";

describe("accesslint-023", () => {
  it("reports type=button with empty value", () => {
    const doc = makeDoc('<input type="button" value="">');
    const v = accesslint023.run(doc);
    expect(v).toHaveLength(1);
    expect(v[0].ruleId).toBe("accesslint-023");
  });
  ...
  it("passes type=button with value", () => {
    ...
    expect(accesslint023.run(doc)).toHaveLength(0);
  });
